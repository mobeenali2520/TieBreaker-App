    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-1.5 bg-slate-950 p-1.5 rounded-xl border border-slate-800">
          ...
          <button
            onClick={() => { setMode('pairwise'); resetPairwise(); }}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all ${
              mode === 'pairwise' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
      ...
      {/* MODE 3: PAIRWISE 1V1 */}
      {mode === 'pairwise' && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-xl max-w-2xl mx-auto text-center space-y-6">
          {!pairwiseWinner ? (
            pairwiseList.length >= 2 ? (
              <div className="space-y-6">
                <div>
                  <span className="text-xs uppercase font-bold tracking-wider text-indigo-400 bg-indigo-950/60 px-3 py-1 rounded-full border border-indigo-700/50">
                    Gut Check Face-off
                  </span>
                  <h3 className="text-lg font-bold text-white mt-2">
                    Which choice feels better right now?
                  </h3>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <button
                    onClick={() => selectPairwiseWinner(pairwiseList[0])}
                    className="p-6 rounded-2xl bg-gradient-to-br from-slate-800 to-indigo-950/40 border-2 border-slate-700 hover:border-indigo-500 hover:scale-105 transition-all text-left space-y-2 group"
                  >
                    <div className="flex items-center gap-2">
                      <span className="w-3 h-3 rounded-full" style={{ backgroundColor: pairwiseList[0]?.color || '#6366f1' }} />
                      <span className="text-xs text-indigo-300 font-bold uppercase">Choice A</span>
                    </div>
                  ...
                  <button
                    onClick={() => selectPairwiseWinner(pairwiseList[1])}
                    className="p-6 rounded-2xl bg-gradient-to-br from-slate-800 to-purple-950/40 border-2 border-slate-700 hover:border-purple-500 hover:scale-105 transition-all text-left space-y-2 group"
                  >
                    <div className="flex items-center gap-2">
                      <span className="w-3 h-3 rounded-full" style={{ backgroundColor: pairwiseList[1]?.color || '#a855f7' }} />
                      <span className="text-xs text-purple-300 font-bold uppercase">Choice B</span>
                    </div>
                    <div className="text-base font-bold text-white group-hover:text-purple-300">
                      {pairwiseList[1]?.name}
                    </div>
                  </button>
                </div>
              ...
              <p className="text-slate-400 text-xs">Need at least 2 options.</p>
            )
          ) : (
